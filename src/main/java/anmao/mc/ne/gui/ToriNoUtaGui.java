...
public class ToriNoUtaGui {
    ...
    public static final boolean showEffects = EnchantmentsConfig.INSTANCE.getBoolean(EnchantReg.TORI_NO_UTA,"showEffects");
    private static final float quota = EnchantmentsConfig.INSTANCE.getValue(EnchantReg.TORI_NO_UTA,"quota");
    ...
    public ToriNoUtaGui(Minecraft pMinecraft) {
    ...
    public void render(GuiGraphics guiGraphics, DeltaTracker pPartialTick) {
        if (ToriNoUta.ENABLE && showEffects){
            ...
            if (player != null){
                ...
                if (!itemStack.isEmpty() && EnchantHelper.hasEnchant(itemStack, Enchants.toriNoUta) && player.getOffhandItem().getItem() == Items.FEATHER){
